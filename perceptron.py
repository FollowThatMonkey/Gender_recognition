...
clf = linear_model.Perceptron(alpha=1e-15, fit_intercept=False, max_iter=10000, n_jobs=-1)
...
plt.gcf().suptitle('Tablica pomyłek dla klasyfikatora Perceptron')
plt.savefig('wykresy/perc_conf.png')
plt.show()

## KRZYWA ROC
roc = metrics.plot_roc_curve(best_clf, X_test, y_test)
plt.gcf().suptitle('Krzywa ROC dla klasyfikatora Perceptron')
plt.savefig('wykresy/perc_roc.png')
plt.show()