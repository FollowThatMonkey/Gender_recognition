...
C = np.linspace(60, 65, 80)
fit_intercept = (False,)
parameters = {'C': C, 'fit_intercept': fit_intercept, 'max_iter': (10000,)}

score = model_selection.GridSearchCV(clf, parameters, n_jobs=-1)
score.fit(X, y)

with open('wyniki/pass_aggres_najlepszy.csv', 'w') as f:
    w = csv.writer(f)
    for key, val in score.best_params_.items():
        w.writerow([key, val])
        
with open('wyniki/pass_aggres_wyniki.csv', 'w') as f:
    w = csv.writer(f)
