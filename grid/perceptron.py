...
alpha = np.linspace(1e-15, 1e-9, 90)
fit_intercept = (True, False)

parameters = {'alpha': alpha, 'fit_intercept': fit_intercept, 'max_iter': (10000,)}
score = model_selection.GridSearchCV(clf, parameters, n_jobs=-1)
score.fit(X, y)

with open('wyniki/perceptron_najlepszy.csv', 'w') as f:
    w = csv.writer(f)
    for key, val in score.best_params_.items():
        w.writerow([key, val])
        
with open('wyniki/perceptron_wyniki.csv', 'w') as f:
    w = csv.writer(f)
