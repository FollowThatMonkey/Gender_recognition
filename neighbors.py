...
clf = neighbors.KNeighborsClassifier(n_neighbors=54, n_jobs=-1)
...
plt.gcf().suptitle('Tablica pomyłek dla klasyfikatora KNeighbors')
plt.savefig('wykresy/neighbors_conf.png')
plt.show()

## KRZYWA ROC
roc = metrics.plot_roc_curve(best_clf, X_test, y_test)
plt.gcf().suptitle('Krzywa ROC dla klasyfikatora KNeighbors')
plt.savefig('wykresy/neighbors_roc.png')
plt.show()