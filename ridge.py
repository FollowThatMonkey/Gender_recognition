...
clf = linear_model.RidgeClassifier(alpha=0.14026845637583893, fit_intercept=False)
...
plt.gcf().suptitle('Tablica pomyłek dla klasyfikatora Ridge')
plt.savefig('wykresy/ridge_conf.png')
plt.show()

## KRZYWA ROC
roc = metrics.plot_roc_curve(best_clf, X_test, y_test)
plt.gcf().suptitle('Krzywa ROC dla klasyfikatora Ridge')
plt.savefig('wykresy/ridge_roc.png')
plt.show()